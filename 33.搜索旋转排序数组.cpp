...
public:
    static int search(vector<int>& nums, int target) {
        ...
        auto mid = lo+(hi-lo)/2;
        while(lo < hi){
            //先用mid把数组划为两部分，想要舍弃掉任意一部分需要判断target在哪一部分
            //而判断target的位置需要将target与mid和数组边界比较
            //target需要二分查找前向规约时有两种情况：
            //n[0]<=target<=n[mid] || n[mid]<=n[0]<=target
            if((target>=*lo) ^ (*mid>=target) ^ (*mid>=*lo)) hi = mid;
            else lo = mid+1;
            mid = lo+(hi-lo)/2;
        }
